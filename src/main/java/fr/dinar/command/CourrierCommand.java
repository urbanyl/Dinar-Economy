...
public final class CourrierCommand {
    ...
    private static int send(CommandContext<ServerCommandSource> ctx, double amount) {
        ...
        if (message.isBlank() || message.length() > 500) {
            ctx.getSource().sendError(Text.literal("§cMessage vide ou trop long (500 caractères max)."));
            return 0;
        }
        ...
        ctx.getSource().sendFeedback(() -> Text.literal("§d✉ §aLa lettre a été envoyée à §e" + ref.displayName()
                + (finalAmount > 0 ? " §7avec §e" + DinarMod.economy.money(finalAmount) : "")), false);
        return 1;
    }
    ...
    private static int list(CommandContext<ServerCommandSource> ctx) {
        ...
        if (mails.isEmpty()) {
            ctx.getSource().sendFeedback(() -> Text.literal("§7Vous n'avez aucune lettre."), false);
            return 0;
        }
        ctx.getSource().sendFeedback(() -> Text.literal("§6§lBoîte aux lettres §r§7(§f" + mails.size() + "§7)"), false);
        for (MailEntry m : mails) {
            ...
            ctx.getSource().sendFeedback(() -> Text.literal("§f#" + m.id + " §7" + marker + "§e" + sender + moneyMark
                    + " §7» §f" + msg), false);
        }
        ctx.getSource().sendFeedback(() -> Text.literal("§7Lire : §f/courrier lire <id>"), false);
        return 1;
    }
    ...
    private static int read(CommandContext<ServerCommandSource> ctx) {
        ...
        if (m == null) {
            ctx.getSource().sendError(Text.literal("§cLettre introuvable."));
            return 0;
        }
        ...
        ctx.getSource().sendFeedback(() -> Text.literal("§d✉ §6Lettre de §e" + sender + " §7(#" + m.id + ")"), false);
        ctx.getSource().sendFeedback(() -> Text.literal("§7» §f" + msg), false);
        if (hadMoney) {
            ctx.getSource().sendFeedback(() -> Text.literal("§a+ Vous avez récupéré §e"
                    + DinarMod.economy.money(m.attachedMoney) + " §a(contenu de la lettre)."), false);
        }
        return 1;
    }
    ...
    private static int delete(CommandContext<ServerCommandSource> ctx) {
        ...
        ctx.getSource().sendFeedback(() -> Text.literal("§7Lettre supprimée."), false);
        return 1;
    }
    ...
    private static int cancel(CommandContext<ServerCommandSource> ctx) {
        ...
        ctx.getSource().sendFeedback(() -> Text.literal("§7Lettre annulée (argent éventuel récupéré)."), false);
        return 1;
    }

    private static int help(CommandContext<ServerCommandSource> ctx) {
        ctx.getSource().sendFeedback(() -> Text.literal("§6§lCourrier §7» §f/courrier envoyer <joueur> <message> "
