...
public final class PoliceCommand {
    ...
    private static int add(CommandContext<ServerCommandSource> ctx) {
        ...
        ctx.getSource().sendFeedback(() -> Text.literal("§a" + ref.displayName() + " est maintenant policier."), false);
    ...
    private static int remove(CommandContext<ServerCommandSource> ctx) {
        ...
        ctx.getSource().sendFeedback(() -> Text.literal("§a" + ref.displayName() + " n'est plus policier."), false);
    ...
    private static int list(CommandContext<ServerCommandSource> ctx) {
        ...
        if (officers.isEmpty()) {
            ctx.getSource().sendFeedback(() -> Text.literal("§7Aucun policier."), false);
            return 0;
        }
        ctx.getSource().sendFeedback(() -> Text.literal("§9§lPolice §r§7(§f" + officers.size() + "§7)"), false);
        for (String name : officers) {
            ctx.getSource().sendFeedback(() -> Text.literal("§f• §e" + name), false);
        }
        return 1;
    }

    private static int help(CommandContext<ServerCommandSource> ctx) {
        ctx.getSource().sendFeedback(() -> Text.literal("§6§lPolice §7» §f/police ajouter <joueur> "
